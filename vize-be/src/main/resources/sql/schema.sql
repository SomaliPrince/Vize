...
CREATE TABLE boards
(
    board_code VARCHAR(5) PRIMARY KEY, --pol, biz, mu
...
(
    ...
    board_id   VARCHAR(5) REFERENCES boards (board_code) ON DELETE RESTRICT DEFERRABLE,
    op_post_id INTEGER,
...
(
    id         SERIAL PRIMARY KEY,
    is_op      BOOLEAN,
    board_id   VARCHAR(5) NOT NULL REFERENCES boards (board_code) ON DELETE RESTRICT,
